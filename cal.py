...
def arpdanger(a): 
    global arpd 
    arpd=ImageTk.PhotoImage(Image.open("danger.png"))
    Z=Label(frame_b3,image=arpd)
    Z.place(relx=0.05,rely=0.30) 
    lb1=Label(frame_b3,text="[!] you are under attack, \n REAL-MAC: "+str(a[0])+", \n FACE MAC:"+str(a[1]),fg="black",font="times 16 bold").place(relx=0.05,rely=0.89)
    
#function if no spoofing is taking place 
def arpsafe(): 
    print("arpsafe")
    arps=ImageTk.PhotoImage(Image.open("safe.jpeg"))
    Z=Label(frame_b3,image=arps)
    Z.place(relx=0.05,rely=0.30) 
    lb1=Label(frame_b3,text="you are safe",fg="black",font="times 24 bold").place(relx=0.05,rely=0.90)
...
def confirm(x):
    ...
    C=Label(frame_b3,text="Should I start the detection mechanism "+str(x)).place(relx=0.05,rely=0.03)
    button1=Button(frame_b3,text="Yes",width=5,height=1,command=lambda:go(x)).place(relx=0.05,rely=0.11)
    button2=Button(frame_b3,text="close",width=5,height=1,command=lambda:root.destroy()).place(relx=0.30,rely=0.11)
    button3=Button(frame_b3,text="reset",width=5,height=1,command=reset).place(relx=0.55,rely=0.11)
